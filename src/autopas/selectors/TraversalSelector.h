...
std::unique_ptr<TraversalInterface> TraversalSelector<ParticleCell>::generateTraversal(
  ...
  switch (traversalType) {
    ...
    case TraversalOption::vl_list_iteration: {
      return std::make_unique<VLListIterationTraversal<ParticleCell, Functor, dataLayout, useNewton3>>(
          &functor);
    }
    // Var Verlet Lists
    case TraversalOption::vvl_as_built: {
      return std::make_unique<VVLAsBuildTraversal<ParticleCell, typename ParticleCell::ParticleType, Functor,
                                                  dataLayout, useNewton3>>(&functor);
    }
    // Verlet List Cells
